...
def crop_image(input_image, output_image,location):
    ...
    bottom (int): 下边界的像素坐标
    """
    left, top, right, bottom = location[0]
...
def jiance(screenshot_path):
    ...
    if below_threshold_ratio < 0.6 and width < height:
        print("有广告")
...
def CloseAd(screenshot_path):
    locations = [[[70,168,100,190],'cross']]
      #[[70,168,100,190],'cross'],[[17,73,49,105],'circle'],
    for location in locations:
        crop_image(screenshot_path,'cache_croped.png',location)
        std_distance = Dispersion('cache_croped.png','cache_TwoColor.png',max_difference=15,min_rgb_value=70,shape=location[1])
        if std_distance:
            dlg.click_input(coords=(int((location[0][0]+location[0][2])/2),int((location[0][1]+location[0][3])/2)))
